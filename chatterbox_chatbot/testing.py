...
class Testing:
    ...
    def response(self,sentence,userID='Suvrodip'):
        ...
        if results:
            while results:
                ...
                results.pop(0)
        #if ans contains some value then return response to user's query else return a random fallback message
        fallback_messages = [
            "I'm sorry, could you rephrase?",
            "Sorry! I am still learning. You can train me by providing more data."
        ]
        return ans if ans != "" else random.choice(fallback_messages)
